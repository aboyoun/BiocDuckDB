...
titanic_tf <- tempfile()
writeArray(Titanic, titanic_tf, datacol = "fate")
...
state_tables <- list(dim1 = data.frame(region = state.region,
                     ...
                     dim2 = NULL)
state_tf <- tempfile()
writeArray(state.x77, state_tf, dimtbls = state_tables)
...
for (lib in sort(unique(colData(zilionis)[["Library"]]))) {
    ...
    cnts <- counts(sce)
    writeArray(cnts, path, keycols = c("Genename", "Barcode"), datacol = "Value")
    ...
    for (rd in reducedDimNames(sce)) {
        ...
        if (nrow(mat) > 0L) {
            writeArray(mat, path, keycols = c("Barcode", "Dim"), datacol = "Value")
        }
    }
}
```
