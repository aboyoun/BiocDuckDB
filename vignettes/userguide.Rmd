...
writeCoordArray(counts(zilionis), zilionis_counts_path,
                keycols = c("Genename", "Barcode"), datacol = "Value",
                dimtbls = dimtbls)
...
tbl_ddb <- DuckDBTable(zilionis_cdata_path, keycols = c("Library", "Barcode"))
tbl <- tblconn(tbl_ddb, filter = FALSE)
...
tbl_ddb <- DuckDBTable(zilionis_counts_path,
                       keycols = c("Genename", "Library", "Barcode"))
tbl <- tblconn(tbl_ddb, filter = FALSE)
tbl <- mutate(tbl, ID = concat_ws("_", Library, Barcode))
...
zilionis_rdims <-
    sapply(reducedDimNames(zilionis), function(rd) {
        path <- file.path(zilionis_rdims_path, sprintf("type=%s", rd))
        tbl_ddb <- DuckDBTable(path, keycols = c("Library", "Barcode", "Dim"))
        tbl <- tblconn(tbl_ddb, filter = FALSE)
        tbl <- mutate(tbl, ID = concat_ws("_", Library, Barcode))
        mat <- DuckDBMatrix(tbl, row = list("ID" = rownames(zilionis_cdata)),
