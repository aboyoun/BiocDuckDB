...
# Create a list for the dimention table containing the partitioning
dimtbls <- DataFrameList(rowname = DataFrame(region = state[["region"]],
                                             division = state[["division"]],
                                             row.names = state[["rowname"]]))
...
western_state_ddb <- DuckDBMatrix(file.path(state_tf, "region=West"), row = "rowname", col = "colname", datacol = "value")
...
southern_state_ddb <- DuckDBMatrix(southern_df, row = "rowname", col = "colname", datacol = "value")
...
for (lib in sort(unique(colData(zilionis)[["Library"]]))) {
    sce <- zilionis[, colData(zilionis)[["Library"]] == lib]

    partition <- sprintf("Library=%s", lib)

    # Column metadata
    path <- file.path(zilionis_cdata_path, partition)
    cdata <- colData(sce)
    cdata[["Library"]] <- NULL
    arrow::write_dataset(as.data.frame(cdata), path, format = "parquet")

    # Counts matrix
    path <- file.path(zilionis_counts_path, partition)
    cnts <- counts(sce)
    writeArray(cnts, path, dim_names = c("Genename", "Barcode"), value_name = "Value")

    # Reduced dimensions
    for (rd in reducedDimNames(sce)) {
        path <- file.path(zilionis_rdims_path, sprintf("type=%s", rd), partition)
        mat <- na.omit(reducedDim(sce, rd))
        if (nrow(mat) > 0L) {
            writeArray(mat, path, dim_names = c("Barcode", "Dim"), value_name = "Value")
        }
    }
}
```
...
zilionis_coldata <- DuckDBDataFrame(tbl, datacols = setdiff(colnames(tbl), "ID"), keycols = "ID")
zilionis_coldata <- zilionis_coldata[sort(rownames(zilionis_coldata)), ]
...
zilionis_counts <- DuckDBMatrix(tbl, row = "Genename",
                                ...
                                datacol = "Value")
zilionis_counts <- zilionis_counts[sort(rownames(zilionis_counts)), ]
```
