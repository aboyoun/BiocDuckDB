...
titanic <- do.call(expand.grid, c(dimnames(Titanic), stringsAsFactors = TRUE))
titanic$fate <- as.integer(Titanic[as.matrix(titanic)])
titanic_tf <- tempfile(fileext = ".parquet")
arrow::write_parquet(titanic, titanic_tf)
...
list.files(state_tf, recursive = TRUE)
...
```{r}
set.seed(123)
ind <- arrayInd(sort(sample(26 * 26 * 12, 1000)), c(26, 26, 12))
