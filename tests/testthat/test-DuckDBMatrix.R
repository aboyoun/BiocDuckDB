...
test_that("basic methods work as expected for a DuckDBMatrix", {
    ...
    pqmat <- DuckDBMatrix(state_path, datacols = "value", row = "rowname", col = "colname")
    ...
    pqmat <- DuckDBMatrix(state_path, datacols = "value", row = dimnames(state.x77)[1L], col = "colname")
    expect_s4_class(pqmat, "DuckDBMatrix")
    expect_identical(type(pqmat), "double")
    expect_identical(type(pqmat), typeof(state.x77))
    expect_identical(length(pqmat), length(state.x77))
    ...
    pqmat <- DuckDBMatrix(state_path, datacols = "value", row = "rowname", col = dimnames(state.x77)[2L])
    expect_s4_class(pqmat, "DuckDBMatrix")
    expect_identical(type(pqmat), "double")
    expect_identical(type(pqmat), typeof(state.x77))
    expect_identical(length(pqmat), length(state.x77))
    ...
    pqmat <- DuckDBMatrix(state_path, datacols = "value", row = dimnames(state.x77)[1L], col = dimnames(state.x77)[2L])
    checkDuckDBMatrix(pqmat, state.x77)

    pqmat <- DuckDBMatrix(state_path, datacols = "value", keycols = dimnames(state.x77))
    checkDuckDBMatrix(pqmat, state.x77)
})
