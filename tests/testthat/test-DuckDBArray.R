...
test_that("basic methods work as expected for a DuckDBArray", {
    ...
    pqarray <- DuckDBArray(titanic_path, key = dimnames(titanic_array), fact = "fate", type = "double")
    ...
    pqarray <- DuckDBArray(titanic_path, key = dimnames(titanic_array), fact = "fate", type = "character")
    expect_s4_class(pqarray, "DuckDBArray")
...
test_that("basic methods work as expected for a sparse DuckDBArray", {
    pqarray <- DuckDBArray(sparse_path, key = list(dim1 = LETTERS, dim2 = letters, dim3 = month.abb), fact = "value")
    checkDuckDBArray(pqarray, sparse_array)
    expect_true(is_sparse(pqarray))

    pqarray <- DuckDBArray(sparse_path, key = list(dim1 = LETTERS, dim2 = letters, dim3 = month.abb), fact = "value", type = "double")
    expect_s4_class(pqarray, "DuckDBArray")
    expect_identical(type(pqarray), "double")
    ...
    pqarray <- DuckDBArray(sparse_path, key = list(dim1 = LETTERS, dim2 = letters, dim3 = month.abb), fact = "value", type = "character")
    expect_s4_class(pqarray, "DuckDBArray")
    expect_identical(type(pqarray), "character")
    expect_identical(length(pqarray), length(sparse_array))
    expect_identical(dim(pqarray), dim(sparse_array))
...
test_that("aperm and t methods work as expected for a DuckDBArray", {
    seed <- DuckDBArray(titanic_path, key = dimnames(titanic_array), fact = "fate")
    ...
    seed <- DuckDBArray(state_path, key = dimnames(state.x77), fact = "value")
...
test_that("Math methods work as expected for a DuckDBArray", {
    names(dimnames(state.x77)) <- c("rowname", "colname")
    pqarray <- DuckDBArray(state_path, key = dimnames(state.x77), fact = "value")
